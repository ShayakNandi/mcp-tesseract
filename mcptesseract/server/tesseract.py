...
def ocr_img2txt():
    """
    Perform OCR on all PNG images in the image_folder directory and save the transcriptions as text files.

    This tool reads all .png files from the image_folder directory, performs OCR using pytesseract, and writes the resulting text to the results/txt/ocr-img2txt/pytesseract directory. Each output file is named after the original image file with a .txt extension.
...
@mcp.tool()
async def openai_ocr_llm_async():
    """
    Run the OpenAI LLM post-processing pipeline on OCR outputs asynchronously.

    This tool fetches all OCR output text files and corresponding images, then runs the OpenAI LLM (img+txt to txt) pipeline asynchronously, saving results to the ocr-llm-img2txt directory.
    ...
    await process_double_async(
        img_filepaths,
        ocr_output_filepaths,
        txt_output_dir / "ocr-llm-img2txt",
        openai_img_txt2txt_async,
        llms["openai"],
    )


@mcp.tool()
async def openai_llm_img2txt():
    """
    Run the OpenAI LLM image-to-text pipeline asynchronously.

    This tool processes all images using the OpenAI LLM (image to text) pipeline asynchronously and saves the results to the llm-img2txt directory.
    ...
    try:
        await process_single_async(
            img_filepaths,
            txt_output_dir / "llm-img2txt",
            openai_img2txt_async,
            llms["openai"],
        )
    except Exception as e:
        print(f"Error in openai_llm_img2txt: {e}")
        return f"Error: {e}"


@mcp.tool()
async def openai_llm_img2json():
    """
    Run the OpenAI LLM image-to-JSON pipeline asynchronously.

    This tool processes all images using the OpenAI LLM (image to JSON) pipeline asynchronously and saves the results to the llm-img2json directory.
    ...
    Returns:
        None. Writes output files to disk.
    """
    await process_json_async(
        img_filepaths,
        json_output_dir / "llm-img2json",
        openai_img2json_async,
        llms["openai"],
    )


@mcp.tool()
async def openai_llm_txt2json():
    """
    Run the OpenAI LLM text-to-JSON pipeline asynchronously on OCR+LLM outputs.

    This tool fetches all text files from the ocr-llm-img2txt/gpt-4o directory and processes them using the OpenAI LLM (text to JSON) pipeline asynchronously, saving results to the llm-txt2json directory.
    ...
    await process_json_async(
        txt_filepaths,
        json_output_dir / "llm-txt2json",
        openai_txt2json_async,
        llms["openai"],
    )


@mcp.tool()
async def gemini_ocr_llm_async():
    """
    Run the Gemini LLM post-processing pipeline on OCR outputs asynchronously.

    This tool fetches all OCR output text files and corresponding images, then runs the Gemini LLM (img+txt to txt) pipeline asynchronously, saving results to the ocr-llm-img2txt directory.
    ...
    await process_double_async(
        ...
        ocr_output_filepaths,
        txt_output_dir / "ocr-llm-img2txt",
        gemini_img_txt2txt_async,
        llms["google"],
    )


@mcp.tool()
async def gemini_llm_img2txt():
    """
    Run the Gemini LLM image-to-text pipeline asynchronously.

    This tool processes all images using the Gemini LLM (image to text) pipeline asynchronously and saves the results to the llm-img2txt directory.
    ...
    await process_single_async(
        img_filepaths,
        txt_output_dir / "llm-img2txt",
        gemini_img2txt_async,
        llms["google"],
    )


@mcp.tool()
async def gemini_llm_img2json():
    """
    Run the Gemini LLM image-to-JSON pipeline asynchronously.

    This tool processes all images using the Gemini LLM (image to JSON) pipeline asynchronously and saves the results to the llm-img2json directory.
    ...
    await process_json_async(
        img_filepaths,
        json_output_dir / "llm-img2json",
        gemini_img2json_async,
        llms["google"],
    )


@mcp.tool()
async def gemini_llm_txt2json():
    """
    Run the Gemini LLM text-to-JSON pipeline asynchronously on OCR+LLM outputs.

    This tool fetches all text files from the ocr-llm-img2txt/gemini-2.5-flash directory and processes them using the Gemini LLM (text to JSON) pipeline asynchronously, saving results to the llm-txt2json directory.
...
def ocr_image_to_text(image_path: str, output_folder: str = "transcriptions") -> str:
    ...
    try:
        ...
        return (
            f"Successfully processed {image_path}. Transcription saved to {output_path}"
        )
...
def store_word_frequencies(txt_file_path: str, db_path: str = "word_freq.db") -> str:
    ...
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS word_freq (word TEXT PRIMARY KEY, count INTEGER)"
    )
...
def get_all_word_frequencies(db_path: str = "word_freq.db", limit: int = 20) -> str:
    ...
    cursor.execute(
        "SELECT word, count FROM word_freq ORDER BY count DESC LIMIT ?", (limit,)
    )
...
def parse_bibliography_entry(entry_text: str) -> dict[str, str | int | None] | None:
    ...
    if (
        library_match
        and result["library"]
        and isinstance(result["library"], str)
        and result["library"] in middle_parts
    ):
...
def process_ground_truth_folder(
    folder_path: str = "ground_truth", db_path: str = "bibliography.db"
) -> str:
    """
                ...
                for entry_text in entries:
                    ...
                    if parsed_entry:
                        try:
                            ...
                            failed_entries += 1
                            print(
                                f"Failed to insert entry: {entry_text[:50]}... Error: {e}"
                            )
...
@mcp.tool()
def query_bibliography(
    query: str, db_path: str = "bibliography.db", limit: int = 10
) -> str:
    """
...
def get_bibliography_stats(db_path: str = "bibliography.db") -> str:
    ...
    try:
        ...
        # Get year range
        cursor.execute(
            "SELECT MIN(year), MAX(year) FROM bibliography WHERE year IS NOT NULL"
        )
...
@mcp.tool()
def search_by_topic(
    topic: str, db_path: str = "bibliography.db", limit: int = 15
) -> str:
    """
...
@mcp.tool()
def display_all_bibliography_entries(
    db_path: str = "bibliography.db", format: str = "detailed"
) -> str:
    """
        ...
        if format == "detailed":
            ...
            for i, entry in enumerate(entries, 1):
                ...
                if place or publisher:
                    output += (
                        f"Published: {place}{': ' + publisher if publisher else ''}\n"
                    )
                if pages:
...
def clear_bibliography(db_path: str = "bibliography.db") -> str:
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='bibliography'"
        )
        if cursor.fetchone() is None:
            conn.close()
            return (
                f"Table 'bibliography' does not exist in {db_path}. Nothing to delete."
            )
...
@mcp.tool()
def process_json_ground_truth(
    json_folder: str = "json_truth", db_path: str = "bibliography.db"
) -> str:
    """
                ...
                if "entries" in data and isinstance(data["entries"], list):
                    for entry in data["entries"]:
                        try:
                            ...
                            failed_entries += 1
                            print(
                                f"Failed to insert entry from {source_filename}: {str(e)}"
                            )
...
@mcp.tool()
def query_json_bibliography(
    query: str, db_path: str = "bibliography.db", limit: int = 20
) -> str:
    """
...
def search_by_author(author_name: str, db_path: str = "bibliography.db") -> str:
    ...
    try:
        ...
        for i, row in enumerate(results, 1):
            ...
            formatted_results += (
                f"{i}. {author}{birthyear}{deathyear} - {title}{publishyear}\n"
            )
...
def search_by_year_range(
    start_year: int, end_year: int, db_path: str = "bibliography.db"
) -> str:
    """
...
def get_json_bibliography_stats(db_path: str = "bibliography.db") -> str:
    ...
    try:
        ...
        cursor.execute(
            "SELECT COUNT(*) FROM bibliography WHERE publishyear IS NOT NULL"
        )
        with_years = cursor.fetchone()[0]

        # Get publication year range
        cursor.execute(
            "SELECT MIN(publishyear), MAX(publishyear) FROM bibliography WHERE publishyear IS NOT NULL"
        )
        year_range = cursor.fetchone()

        # Get birth year range
        cursor.execute(
            "SELECT MIN(birthyear), MAX(birthyear) FROM bibliography WHERE birthyear IS NOT NULL"
        )
...
@mcp.tool()
def display_all_json_bibliography(
    db_path: str = "bibliography.db", format: str = "compact", limit: int = 100
) -> str:
    """
    ...
    try:
        ...
        if format == "detailed":
            ...
            for i, entry in enumerate(entries, 1):
                ...
                deathyear = (
                    f", d. {entry[4]})" if entry[4] else (")" if birthyear else "")
                )
